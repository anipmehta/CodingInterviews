...
class Supervisor{
    ...
    public void updateTravellerStatuses(){
        for(Traveller traveller : travellers){
            traveller.nextHeathState(checkIfTravellersLocationIsCompromised(traveller));
        }
    }
    public boolean checkIfTravellersLocationIsCompromised(Traveller traveller){
        List<Traveller> travellersAtLocation = getLocationTravellersMap().get(traveller.getLocation());
        for(Traveller person : travellersAtLocation){
            if(person.getName().equals(traveller.getName())){
                continue;
            }
            if(!person.getHealthStatus().equals("HEALTHY")){
                return true;
            }
        }
        return false;
